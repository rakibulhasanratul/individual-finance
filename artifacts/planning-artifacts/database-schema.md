...
- Added `audit_logs` table for application-level trace logging
...
audit_logs              group_member_permissions group_goal_reservations
personal_incomes                                group_goal_implementations
personal_expenses                               over_withdrawal_borrowings
personal_goals ─────── personal_goal_reservations over_withdrawal_settlements
personal_goal_implementations                     group_goals
personal_loans ─────── personal_loan_repayments
...
| `canInviteMember`       | `BOOLEAN`     | NOT NULL, DEFAULT false           | Can invite new members via email |
| `canRemoveMember`        | `BOOLEAN`     | NOT NULL, DEFAULT false           | Can remove members from group |
| `canDeposit`             | `BOOLEAN`     | NOT NULL, DEFAULT true            | Can record deposits |
| `canWithdraw`           | `BOOLEAN`     | NOT NULL, DEFAULT true            | Can record withdrawals |
| `canReserveForGoal`      | `BOOLEAN`     | NOT NULL, DEFAULT false           | Can reserve funds for goals |
| `canImplementGoal`     | `BOOLEAN`     | NOT NULL, DEFAULT false          | Can implement goals (spend reserved money) |
| `canSettleObligation`   | `BOOLEAN`     | NOT NULL, DEFAULT true            | Can settle borrowing obligations |
| `canUpdatePermissions`  | `BOOLEAN`     | NOT NULL, DEFAULT false           | Can modify member permissions |
| `canViewAudit`          | `BOOLEAN`     | NOT NULL, DEFAULT true            | Can view audit trail |
...
- Most permissions default to `true` (deposit, withdraw, settle, view audit) for open participation.
- Sensitive permissions (reserve, implement, update permissions) default to `false`.
- Only admins with `canUpdatePermissions = true` can modify others' permissions.
...
Application-level trace logs for debugging and observability.

| Column        | Type          | Constraints             | Notes                                                              |
| ------------- | ------------- | ----------------------- | ------------------------------------------------------------------ |
| `uuid`        | `UUID`        | PK                      |                                                                    |
| `action`      | `TEXT`        | NOT NULL                | Action/operation name (e.g., `group.deposit.created`)                  |
| `traceId`     | `TEXT`        | NOT NULL, INDEX          | Correlation ID for tracing related operations                        |
| `payload`     | `JSON`        | NOT NULL                | Log data including inputs, outputs, step status                      |
| `createdAt`  | `TIMESTAMPTZ` | NOT NULL, DEFAULT now() |                                                                    |

**Indexes:**

- `traceId` (for filtering by trace/correlation ID)

**Design notes:**

- Append-only application logs for operational debugging.
- Lightweight alternative to `audit_events` for tracing.
- Includes step-level success/failure status for each operation path.
- Timestamps in format: `[timestamp] [log level] function [function_name], variable [variable_name] changed its value to [new_value]`
